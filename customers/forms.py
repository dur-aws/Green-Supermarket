...
class CustomerForm(forms.ModelForm):
    class Meta:
        model = Customer
        fields = ['customer_code', 'customer_name', 'customer_type', 'pan_vat_number', 'phone', 'email', 'address', 'status']
        widgets = {
            'customer_code': forms.TextInput(attrs={'class': 'form-control', 'placeholder': 'e.g. CUST-0001'}),
            'customer_name': forms.TextInput(attrs={'class': 'form-control'}),
            'customer_type': forms.Select(attrs={'class': 'form-control'}),
            'pan_vat_number': forms.TextInput(attrs={'class': 'form-control', 'placeholder': '9-digit PAN/VAT No'}),
...
class QuickCustomerForm(forms.ModelForm):
    class Meta:
        model = Customer
        fields = ['customer_name', 'phone', 'email', 'pan_vat_number']

    def save(self, commit=True):
        customer = super().save(commit=False)
        customer.customer_type = Customer.TYPE_REGISTERED
        last_customer = Customer.objects.order_by('-customer_id').first()
        next_id = (last_customer.customer_id + 1) if last_customer else 1
        customer.customer_code = f"CUST-{next_id:04d}"
        if commit:
            customer.save()
        return customer